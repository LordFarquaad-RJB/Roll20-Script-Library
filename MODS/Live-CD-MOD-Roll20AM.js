...
//		log('  >Tracks updated<');
    ...
    outputConfig = function(menu,who,filter){
        var mvTextButton,tracksButton,playlistButton,restrictButton,volumeButton,muteButton,playButton,addButton,increaseButton,decreaseButton,fadeInButton,tagButton,deleteButton,APIButton,tagButton,templateButton,commandButton,rollButton,textButton,accessButton,modeButton,importButton,backButton,fadeOutButton,tagDeleteButton,
        output,outputWho;
    ...
    outputTrack = function(track,menu){
        ...
        }else if (track.mode == "sequence" ){
            modeButton = makeImageButton('!roll20AM --edit,mode,together|'+encodeURIComponent(track.title)+' --config,menu='+menu,singleImage,'Set the playlist playmode; currently sequence','white',15);
        }else{
            modeButton = makeImageButton('!roll20AM --edit,mode,random,single|'+encodeURIComponent(track.title)+' --config,menu='+menu,togetherImage,'Set the playlist playmode; currently together','white');
        }
    ...
    outputList = function(list,menu,mainMenu){  
       ...
       if(list.playing){
        ...
        if (list.mode == "randomSingle"){
            ...
            modeButton = makeImageButton('!roll20AM --edit,mode,sequence|'+encodeURIComponent(list.name)+' --config,menu='+mainMenu,loopImage,'Set the playlist playmode; currently loop','white',15);
        }else if (list.mode == "sequence" ){
            modeButton = makeImageButton('!roll20AM --edit,mode,together|'+encodeURIComponent(list.name)+' --config,menu='+mainMenu,singleImage,'Set the playlist playmode; currently sequence','white',15);
        }else{
	...
	cmdExtract = function(cmd){
	    ...
	    _.each(command.replace(cmdSep.action+',','').split(','),(d)=>{
            vars=d.match(/(volTick|time|volume|delay|level|menu|mode|restrict|tag|API|access|playlistMode|trackMode|tag|tag1|tag2|tag3|tag4|tag5|set|unset|filter|delayed|viewBy|display|restart|sequence)(?:\:|=)([^,]+)/) || null;
            if(vars){
    ...
    trackHandler = function(cmdDetails,trackDetails,who,restrict){
        ...
        //play depending on mode // Custom Dev Start
        if (cmdDetails.details.play){
            if(cmdDetails.details.nostop) {
                // NOSTOP: Play new track and restart any currently playing tracks.
                // This is a Roll20 API limitation - we can't add a track without restarting others.
                var tracksToPlay = [trackDetails];
                _.each(state.Roll20AM.trackDetails, (t) => {
                    if(t.playing && t.id !== trackDetails.id) {
                        tracksToPlay.push(t);
                    }
                });

                trackDetails.playing = true; // Set new track state to playing
                trackDetails.who = who;

                _.each(tracksToPlay, (t) => {
                    var jbTrack = getJukeBox(t.id);
                    if(jbTrack) {
                        jbTrack.set({ playing: true, softstop: false, volume: t.volume });
                    }
                });
                ...
                // NORMAL PLAY: Stop all other tracks and play the new one.
                
                // Stop other tracks and update state
                _.each(state.Roll20AM.trackDetails, t => {
                    if (t.id !== trackDetails.id && t.playing) {
                        stopTrack(t.id, who); // This will set t.playing = false
                    }
                });

                // Stop all playlists that don't contain this track
                _.each(state.Roll20AM.playLists, list => {
                    if(list.playing && !_.contains(list.trackids, trackDetails.id)){
                        stopList(list, who);
                    }
                });
                // Custom Dev edit start
                var loop = null;
                if(cmdDetails.details.loop || trackDetails.mode === 'loop') {
                    loop = 'loop';
                }
                if(cmdDetails.details.restart) {
                    let jbTrack = getJukeBox(trackDetails.id);
                    if(jbTrack) {
                        jbTrack.set({playing: false, softstop: true});
                    }
                }
    ...
    editHandler = function(cmdDetails,list,trackDetails,who,restrict){
        ...
        if (cmdDetails.details.mode){
            ...
            } else if (cmdDetails.details.sequence){
              mode = 'sequence'
            } else {
	...
	playNextTrack = function(list,listTracks,startVolume,who){
        ...
        if (!found){
            if (list.mode === 'sequence') {
                 ...
                 list.currentTrack = [];
                 if (debug) {
                     log('Sequence finished for playlist: ' + list.name);
                 }
        ...
        if (list.mode === 'sequence') {
            let jbTrack = getJukeBox(nexttrackID);
            if(jbTrack) {
                jbTrack.set({playing: false, softstop: true});
            }
        }
	...
	playTrack = function(trackID,level,loop,who){
	    ...
	    trackDetails.who = who;
        jbTrack.set({playing:true,softstop:false,volume:level, loop: (loop === 'loop')}); // Custom Dev
        if (displayTrack){
            sendChat(who,'Now Playing:' + trackDetails.title ,null,{noarchive:true});
        }    
	}, 
...
// 	},
    ...
    generateHelp = function(){
        ...
        playerHelp = '<div style="border: 2px solid black; background-color: white; padding: 3px 3px;">'
            ...
            +'<b><span style="font-family: serif;">--play,[swap],[loop],[mode:shuffle/trueRandom/together/single],[delay:<i>seconds</i>],[nostop]|track/list to play|track/list to play|...</span></b><br> '
            ...
            +'&'+'#8226'+'; <b><span style="font-family: serif;">nostop - </span></b>Plays the track without stopping any currently playing tracks. This allows for concurrent audio playback.<br>'
            +'</ul>'
            +'</li></ul>'
